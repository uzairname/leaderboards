import { Match, PartialGuildRanking, Player, Vote } from '@repo/db/models'
...
import { ongoing_match_view_sig, ongoingMatchPageData } from './views/pages/ongoing-match-view'
  ...
  await Promise.all([
    ...
    app.discord
      .createMessage(
        thread.id,
        (
          await ongoingMatchPageData(
            app,
            ongoing_match_view_sig.newState({
 ...
 */
export async function castPlayerVote(app: App, match: Match, voting_user_id: string, vote: Vote): Promise<void> {
  const team_players = await match.players()

  // ensure the user is in the match
  const team_index = team_players.findIndex(team => team.some(p => p.player.data.user_id === voting_user_id))
  ...
  const unanimous_win =
    team_votes.every(v => v === Vote.Win || v === Vote.Loss) && team_votes.filter(v => v === Vote.Win).length === 1
