...
import { GuildRankings, Guilds, Matches, MatchPlayers, Players, Rankings, Users } from 'database/schema';
...
async function copy_values() {
  ...
  const match_players = await fromDb.select().from(MatchPlayers)
    ...
    .where(eq(Matches.ranking_id, ranking_id))
  await toDb.insert(MatchPlayers).values(match_players.map(m => m.MatchPlayers)).onConflictDoNothing()
