...
export class Match implements DbObject<MatchSelect> {
  ...
  async players(): Promise<MatchPlayer[][]> {
    if (this.db.cache.match_players.has(this.data.id)) return this.db.cache.match_players.get(this.data.id)!
  ...
  async getSummaryMessage(guild_id: string): Promise<MatchSummaryMessageSelect | undefined> {
    const data = (
      await this.db.drizzle
        ...
        .where(and(eq(MatchSummaryMessages.match_id, this.data.id), eq(MatchSummaryMessages.guild_id, guild_id)))
    )[0]
    return data
  }
  ...
  async updateSummaryMessage(guild_id: string, channel_id: string, message_id: string): Promise<void> {
    try {
      // try to insert
  ...
  async convertPlayersQueryResult(
    ...
    const team_players = Array.from({ length: ranking.data.teams_per_match }, () => [] as MatchPlayer[])
...
export class MatchesManager extends DbObjectManager {
  ...
  async getMany(filters: {
    ...
    const paged_matches = this.db.drizzle
      ...
      .orderBy((filters.earliest_first ? asc : desc)(sql`coalesce(${Matches.time_finished}, ${Matches.time_started})`))
   ...
   */
  async updateMatchPlayers(match_players: { match_id: number; update: MatchPlayer }[]): Promise<void> {
    this.db.debug(`updating ${match_players.length} match players`)
