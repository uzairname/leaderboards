...
import { checkGuildInteraction, hasAdminPerms } from '../../perms'
...
import { recordAndScoreNewMatch } from './scoring/score_matches'

const record_match_base_signature = new AppCommand({
...
const getRecordMatchCommandSignatureInGuild = async (app: App, guild_id: string) => {
  const cmd = new AppCommand({
    ...record_match_base_signature.options,
    options: [
      {
  ...
  return cmd.onCommand(async ctx => {
    return {
...
export const record_match_view = new CustomView(
  recordMatchCmd,
  getRecordMatchCommandSignatureInGuild,
)

async function selectTeamPage(
  app: App,
  ctx: StateContext<typeof record_match_base_signature>,
...
async function onSelectTeam(
  app: App,
  ctx: ComponentContext<typeof record_match_base_signature>,
...
async function sendPlayersConfirmingMatchPage(
  app: App,
  ctx: DeferContext<typeof record_match_base_signature>,
): Promise<void> {
  const interaction = checkGuildInteraction(ctx.interaction)
