...
import { settings_cmd_def } from '../modules/admin/views/commands/settings'
import { matches_command_signature } from '../modules/matches/logging/views/commands/matches'
import { record_match_cmd_signature } from '../modules/matches/management/views/commands/record_match'
import { start_match_cmd_signature } from '../modules/matches/management/views/commands/start_match'
import { challenge_cmd_signature } from '../modules/matches/matchmaking/views/commands/challenge'
import { create_ranking_cmd_signature } from '../modules/rankings/views/commands/create_ranking'
import { rankings_cmd_signature } from '../modules/rankings/views/commands/rankings'
      ...
      fields: [
        {
          ...
          value:
            ...
            `\n- ${await commandMention(app, create_ranking_cmd_signature)}: create one or more rankings.` +
            `\n- ${await commandMention(app, rankings_cmd_signature, guild?.data.id)} to view manage all rankings in this server.`,
        },
        {
          name: `Admin & Settings`,
          value:
            `- ${await commandMention(app, settings_cmd_def)}: configure the bot's settings for this server.` +
        ...
        },
        {
          ...
          value:
            ...
            `This means that if you've played fewer games, the algorithm is less certain about your skill, and your rating is more volatile.` +
        ...
        },
        {
          ...
          value:
            ...
            `\n- ${await commandMention(app, challenge_cmd_signature, guild?.data.id)}: Challenge someone to a 1v1 match. ` +
            `The challenged player has ${app.config.ChallengeTimeoutMs / (1000 * 60)} minutes to accept.` +
            `\n- ${await commandMention(app, start_match_cmd_signature, guild?.data.id)}: Admins can start a match between two players.` +
            `\n- ${await commandMention(app, record_match_cmd_signature, guild?.data.id)}: Admins can directly record the result of a match that has already been played. ` +
        ...
        },
        {
          ...
          value:
            ...
            `\n- ${await commandMention(app, matches_command_signature, guild?.data.id)} \`id\`: View the match history for this server. ` +
