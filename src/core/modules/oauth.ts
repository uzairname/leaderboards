...
export function oauthRedirect(bot: DiscordRESTClient, scopes: OAuth2Scopes[]): Response {
  const state = crypto.randomUUID()
  const url = bot.oauthRedirectURL(config.OAUTH_REDIRECT_URI, scopes, state)
...
export async function oauthCallback(app: App, request: Request): Promise<Response> {
  ...
  if (client_state !== discord_state) {
    sentry.debug(`client_state: ${client_state}`)
    sentry.debug(`discord_state: ${discord_state}`)
  ...
  try {
    ...
    var tokendata = await app.bot.getOauthToken(code, config.OAUTH_REDIRECT_URI)
  } catch (e) {
    sentry.catchAfterResponding(e)
    return new Response(null, { status: 500 })
  }

  await saveUserAccessToken(app.bot, tokendata)
...
export async function saveUserAccessToken(
  ...
  const me = await bot.getOauthUser(token.access_token)
  ...
  if (me.scopes.includes(OAuth2Scopes.RoleConnectionsWrite)) {
    sentry.waitUntil(updateUserRoleConnectionData(bot, token.access_token, 10, 'test'))
  }
}
